...
const mergeToolConfigs = (): Record<string, ToolSettings> => {
  const mergedTools: Record<string, ToolSettings> = {};
  
  // Load tools-config.json
  const toolsConfigPath = path.join(process.cwd(), 'tools-config.json');
  const toolsConfig = loadConfigFile(toolsConfigPath) as ToolsConfig | null;
  ...
  if (ocioConfig && ocioConfig.mcp && ocioConfig.mcp.tools) {
    Object.entries(ocioConfig.mcp.tools).forEach(([toolName, toolConfig]) => {
      ...
      if (mergedTools[toolName] && !mergedTools[toolName].enabled) {
        if ((toolConfig as any).autoMode === true) {
          mergedTools[toolName].enabled = true;
        }
        // Merge settings if they exist
        if ((toolConfig as any).preferences) {
          mergedTools[toolName].settings = {
            ...(mergedTools[toolName].settings || {}),
            ...(toolConfig as any).preferences
          };
        }
      } else if (!mergedTools[toolName]) {
        // Add new tool
        mergedTools[toolName] = {
          enabled: (toolConfig as any).autoMode === true,
          settings: (toolConfig as any).preferences || {}
        };
      }
    });
  }
...
const importToolModule = async (toolName: string): Promise<any> => {
  try {
...
const activateTool = async (toolName: string, settings: any): Promise<ActivationResult> => {
...
const generateSummaryTable = (results: ActivationResult[]): string => {
...
export const activateTools = async (): Promise<ActivationResult[]> => {
...
export const setupPostBootActivation = (): void => {
...
export const manuallyTriggerActivation = async (): Promise<ActivationResult[]> => {
