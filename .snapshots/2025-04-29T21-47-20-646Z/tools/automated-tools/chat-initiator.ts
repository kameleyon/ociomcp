...
export function activate() {
  console.log("[TOOL] chat-initiator activated (vigilant mode)");
}

export async function onFileWrite(event: { path: string; content: string }) {
  console.log(`[TOOL] chat-initiator detected file write to: ${event.path}`);

  try {
    const result = await handleTrackContextSize({ message: `File updated: ${event.path}` });
    const parsed = JSON.parse(result.content[0].text);

    if (parsed.shouldTransition) {
      console.log("[TOOL] Token limit reached after file write — initiating new chat session.");
      await handleInitiateChat({
        projectContext: {
          name: "AutoCheckpoint",
          description: `Auto-created because file ${event.path} was updated.`,
          activeFiles: [event.path],
          currentTask: "Continue working after file update",
        },
        hotkeyUsed: false,
      });
    }
  } catch (err) {
    console.error("[TOOL] Error during file write handling:", err);
  }
}

export async function onSessionStart(session: { id: string; startTime: number }) {
  console.log(`[TOOL] chat-initiator detected session start: ${session.id}`);

  try {
    const result = await handleTrackContextSize({ message: `Session started: ${session.id}` });
    const parsed = JSON.parse(result.content[0].text);

    if (parsed.shouldTransition) {
      console.log("[TOOL] Token limit hit at session start — initiating new chat session.");
      await handleInitiateChat({
        projectContext: {
          name: `Session ${session.id}`,
          description: "Auto-triggered due to session start with large context.",
          activeFiles: [],
          currentTask: "New session continuation",
        },
        hotkeyUsed: false,
      });
    } else {
      await handleInitiateChat({
        projectContext: {
          name: `Session ${session.id}`,
          description: "Auto-initiated session context",
          activeFiles: [],
          currentTask: "Starting fresh session",
        },
        hotkeyUsed: false,
      });
    }
  } catch (err) {
    console.error("[TOOL] Error during session start handling:", err);
  }
}

export async function onCommand(command: { name: string; args: any[] }) {
  const commandName = command?.name || "unknown";
  console.log(`[TOOL] chat-initiator detected command: ${commandName}`);

  if (commandName === "save") {
    try {
      const result = await handleTrackContextSize({ message: "User issued SAVE command." });
      const parsed = JSON.parse(result.content[0].text);

      if (parsed.shouldTransition) {
        console.log("[TOOL] Token threshold exceeded after SAVE — initiating continuation.");
        await handleInitiateChat({
          projectContext: {
            name: "AutoSave Session",
            description: "Checkpoint created after SAVE command.",
            activeFiles: [],
            currentTask: "Continuing after save",
          },
          hotkeyUsed: false,
        });
      }
    } catch (err) {
      console.error("[TOOL] Error during SAVE command handling:", err);
    }
  }
}
import { z } from 'zod';
...
import { handleTrackContextSize, chatTransitionManager } from './auto-continue.js'; // Added import
