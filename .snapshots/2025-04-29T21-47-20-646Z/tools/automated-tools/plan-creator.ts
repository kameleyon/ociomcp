// Auto-generated safe fallback for plan-creator

export function activate() {
  console.log("[TOOL] plan-creator activated (passive mode)");
  initializeRoadmap();
}

export function onFileWrite(event: { path: string; content: string }) {
console.log("[Plan Creator] File write detected. Checking roadmap...");
initializeRoadmap();
}

export function onSessionStart(session: { id: string; startTime: number }) {
console.log("[Plan Creator] Session started. Checking roadmap...");
initializeRoadmap();
}

export function onCommand(command: { name: string; args: any[] }) {
if (command.name === 'plan-creator:generate-roadmap') {
  console.log("[Plan Creator] Manual roadmap generation triggered.");
  return generateInitialRoadmap().then(() => ({
    content: [{
      type: "text",
      text: "Roadmap.md generated successfully."
    }],
  })).catch(error => ({
    content: [{
      type: "text",
      text: `Error generating roadmap: ${error}`
    }],
    isError: true
  }));
}
...
/**
* Initializes or generates a roadmap.md if missing.
*/
async function initializeRoadmap() {
try {
  const roadmapPath = path.join(process.cwd(), 'docs', 'roadmap.md');
  try {
    await fs.readFile(roadmapPath, 'utf8');
    console.log('[Plan Creator] Found existing roadmap.md');
  } catch (err) {
    console.log('[Plan Creator] No roadmap.md found. Generating one...');
    await generateInitialRoadmap();
  }
} catch (e) {
  console.error('[Plan Creator] Error during roadmap initialization:', e);
}
}

/**
* Generates a roadmap.md file with placeholders and guidance.
*/
...
await fs.writeFile(path.join(docsDir, 'roadmap.md'), roadmapTemplate);
console.log('[Plan Creator] Created roadmap.md in /docs');
}
