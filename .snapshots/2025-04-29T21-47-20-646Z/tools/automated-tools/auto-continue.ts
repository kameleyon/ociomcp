...
export function onFileWrite(event: { path: string; content: string }) {
  console.log(`[TOOL] auto-continue detected file write to: ${event.path}`);
  // Pass the file path for potentially more specific context tracking
  handleTrackContextSize({ message: `File written: ${event.path}` })
...
export function onSessionStart(session: { id: string; startTime: number }) {
  console.log(`[TOOL] auto-continue detected session start: ${session.id}`);
  // Pass the session ID for potentially more specific context tracking
  handleTrackContextSize({ message: `Session started: ${session.id}` })
...
export function onCommand (command: { name: string; args: any[] }) {
  const commandName = command ? command.name : "unknown";
  console.log(`[TOOL] auto-continue detected command: ${commandName}`);
  // Pass the command name for potentially more specific context tracking
  handleTrackContextSize({ message: `Command issued: ${commandName}` })
...
export const chatTransitionManager = new ChatTransitionManager();
